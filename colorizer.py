...
def image_a_b_gen_train(batch_size, epochs):
...
def image_a_b_gen_val(batch_size, epochs):
    num_batches = ceil(number/batch_size)
    while epochs:
        for i in range(0, int(num_batches)):
...
history = model.fit_generator(
    image_a_b_gen_train(batch_size, epochs),
    verbose = 2,
    steps_per_epoch= steps_per_epoch,
    epochs= epochs,
    validation_data = image_a_b_gen_val(batch_size, epochs),
    validation_steps = validation_steps)



# performance = model.evaluate_generator(
#     image_a_b_gen_val(batch_size),
...
for i in range(np.shape(color_me)[0]):
        ...
        cur[:,:,0] = color_me[i][:,:,0]
        cur[:,:,1:] = output[i]
        imsave("result_" + str(conversion) + "/img_" +str(format(i, '03d'))+" .jpg", lab2rgb(cur))
